...
subroutine getrainwp(n0w,rrate,kext,salb,asym,nfreq)
  use tables2
  real :: n0w,srate
  real, intent(out) :: kext(nfreq),salb(nfreq), asym(nfreq)
  integer :: imu
  imu=3
  call bisection2(pwc13Table(:,imu),nbins,log10(rrate/10**n0w), i0)
  kext(1:nfreq)=kextTable(i0,1:nfreq,imu)*10.**n0w
  salb(1:nfreq)=salbTable(i0,1:nfreq,imu)
  asym(1:nfreq)=asymTable(i0,1:nfreq,imu)
end subroutine getrainwp


subroutine getsnowp2(n0w,iwc,kext,salb,asym,nfreq)
  use tables2
  real :: n0w,iwc
  real, intent(out) :: kext(nfreq),salb(nfreq), asym(nfreq)
  real  :: dpia13,dpia35,z35,z13,z94,dm
  integer :: imu
  imu=3
